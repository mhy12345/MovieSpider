...
#主要测试程序
def get_url(code,ips=[]):
    """
        投票
        如果因为代理IP不可用造成投票失败，则会自动换一个代理IP后继续投
    """
    try:
        ip = choice(ips)                            #在IPS里面产生随机挑选一个返回
    except:
        return False
    else:
        proxies = {
        ...
        try:
            hz_url = "https://www.baidu.com"   # 某网站的地址，用于测试
        hz_r = requests.get(hz_url,headers=headers2,proxies=proxies)
        if (int(hz_r.status_code) == 200):
            print datetime.datetime.now().strftime('%H:%M:%S')+':当前状态是'+str(hz_r.status_code)
        else:
        ...
        print u"第%s次 时间： [%s] ：当前IP:[%s] (剩余可用代理IP数：%s)" % (code,date,ip,len(ips))
